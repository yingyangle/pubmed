...
try:
	with open('{OUTPUT_FOLDER}/authors_already_searched.json', 'r') as ein:
...
try:
	with open('{OUTPUT_FOLDER}/articles_already_searched.json', 'r') as ein:
...
def save_results(author_fullname, author_articles):
	...
	with open(f'{OUTPUT_FOLDER}/saved_articles/json/{author_fullname}.json', 'w') as aus:
	...
	df.to_csv(f'{OUTPUT_FOLDER}/saved_articles/csv/{author_fullname}.csv', index = None, encoding="utf-8")
	
	# update list of authors already searched
	with open('{OUTPUT_FOLDER}/authors_already_searched.json', 'w') as aus:
		json.dump(authors_already_searched, aus)
	
	# update list of articles already saved
	with open('{OUTPUT_FOLDER}/articles_already_saved.json', 'w') as aus:
		json.dump(articles_already_saved, aus)
	
	# update list of failed authors
	with open('{OUTPUT_FOLDER}/failed_authors.json', 'w') as aus:
